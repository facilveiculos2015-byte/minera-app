...
const SHARE_OG_IMAGE_DEFAULT = INDICACAO_BASE + '/og-familia.png?v=20260923ao';
const SHARE_VIDEO_DEFAULT = 'media/convite-familia-minera.mp4?v=20260923ao';
...
async function familiaShareImageFile() {
    ...
    if (!url) return null;
    try {
        const bust = url + (url.includes('?') ? '&' : '?') + 'share=1&v=20260923ao';
...
async function familiaShareVideoFile() {
    ...
    try {
        ...
        const bust = abs + (abs.includes('?') ? '&' : '?') + 'share=1&v=20260923ao';
