...
function garantirBrandLogo() {
    const root = (typeof APP_ROOT === 'string' ? APP_ROOT : '');
    const src = root + 'logo-escavadeira.png?v=20260923an';
    ...
    function showBrowserNotif(title, body) {
        try {
            ...
            if (Notification.permission === 'granted') {
                new Notification(title, { body: body || '', icon: (typeof APP_ROOT === 'string' ? APP_ROOT : '') + 'logo-escavadeira.png?v=20260923an' });
            }
        } catch (e) { /* ignore */ }
    }
